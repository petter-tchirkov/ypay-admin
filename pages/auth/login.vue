...
<template>
  <div class="flex justify-center items-center h-screen w-screen bg-gray-100">
    ...
    <div class="w-96 p-4 rounded-xl shadow-md bg-white flex flex-col gap-4">
      ...
      <NuxtLink :to="localePath('/auth/register')" class="text-green">{{ $t('auth.register') }}</NuxtLink>
      <Button class="w-full" :label="$t('auth.login')" @click="getAuthToken(email, password)" />
    </div>
  </div>
</template>
